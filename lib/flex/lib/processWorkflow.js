...
const processWorkflow = async (context, registry, config) => {
  ...
  try {
    ...
    const evt = `${context.eventName}.${context.payload.action}`
  ...
  for (const rule of rules) {
    ...
    if (promises) {
      ...
      if (!errorOccurred && comment.isEventSupported(event)) await comment.removeErrorComments(context, comment)
    }
  }
}
...
const extractAllActionFromRecipe = (recipe) => {
  const passActions = recipe.pass ? recipe.pass.map(action => action.do) : []
  const failActions = recipe.fail ? recipe.fail.map(action => action.do) : []
  const errorActions = recipe.error ? recipe.error.map(action => action.do) : []
...
const isEventInContext = (event, context) => {
  ...
  const contextEvent = `${context.eventName}.${context.payload.action}`
  const found = eventArray.find(element => {
    if (element.split('.')[1] === '*') {
