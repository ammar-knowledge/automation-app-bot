...
const checkCommonError = (err, context, callFn) => {
  ...
  const errorLog = createLog(context, { callFn, errors: err.toString() })
...
class GithubAPI {
  ...
  static async mergePR (context, callParams) {
    ...
    try {
      ...
      if (err.status === 405 || err.status === 409) {
        ...
        const errorLog = createLog(context, { callFn, errors: err.toString(), logType: logger.logTypes.MERGE_FAIL_ERROR })
  ...
  static async requestReviewers (context, pullNumber, reviewers, teamReviewers) {
    ...
    try {
      ...
      const errorLog = createLog(context, { callFn, errors: err.toString(), logType: logger.logTypes.REQUEST_REVIEW_FAIL_ERROR })
      const log = logger.create(`GithubAPI/${callFn}`)
