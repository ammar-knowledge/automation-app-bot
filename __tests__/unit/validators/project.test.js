...
test('that mergeable is true when PR number is in Project', async () => {
  ...
  const validation = await projects.processValidate(createMockContext({ number: 1 }), settings)
  expect(validation.status).toBe('pass')
})

test('that mergeable is false when PR number is not in Project', async () => {
  const projects = new Project()
  const validation = await projects.processValidate(createMockContext({ number: 3 }), settings)
...
test('test deep validation works', async () => {
  const projects = new Project()
  const validation = await projects.processValidate(createMockContext({ number: 3, description: 'closes #1' }), settings)
  expect(validation.status).toBe('pass')
